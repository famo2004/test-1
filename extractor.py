...
# جدا کردن ساختار اصلی کانفیگ از ریمارک برای بررسی تکراری نبودن
if '#' in config:
...
try:
# پارس کردن URL برای خواندن پارامترها
parsed_url = urllib.parse.urlparse(base_uri)
params = urllib.parse.parse_qs(parsed_url.query)

# فیلتر دوم: بررسی نوع ws (WebSocket)
if params.get('type', [''])[0] == 'ws':

# فیلتر سوم: حذف کانفیگ‌های کاملاً تکراری
if base_uri not in unique_base_uris:
unique_base_uris.add(base_uri)

# تغییر ریمارک به FaMo
final_config = f"{base_uri}#FaMo"
final_configs.append(final_config)
except Exception:
# در صورت خراب بودن ساختار یک لینک، از آن عبور می‌کند
continue

# ذخیره کانفیگ‌ها در قالب یک آرایه استاندارد JSON
# استفاده از indent=4 باعث می‌شود هر کانفیگ تمیز در یک خط مجزا قرار بگیرد
